...
def order_search_text(admin_id='', value=''):
    return ManeuOrder.objects.filter(Q(name__icontains=value, admin_id=admin_id, status=3) | Q(phone__icontains=value, admin_id=admin_id, status=3)).order_by('-time').all()


def order_search_time(admin_id='', timeS='', timeE=''):
    return ManeuOrder.objects.filter(admin_id=admin_id, status=3, time__gte=timeS, time__lte=timeE).order_by('-time').all()


def order_insert(admin_id='', name='', time='', phone='', status='', content='', guest_id='', store_id='', report_id='', remark=''):
    return ManeuOrder.objects.create(name=name, time=time, phone=phone, status=status, guest_id=guest_id, admin_id=admin_id,
