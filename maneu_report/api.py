...
def search_time(request):
    ...
    if admin_id:
        try:
            data = report_search_time(admin_id, request.GET.get('timeS'), request.GET.get('timeE')).values('id',
                                                                                                           'guest_id',
                                                                                                           'name',
                                                                                                           'phone',
                                                                                                           'time',
...
def search_text(request):
    ...
    if admin_id:
        try:
            data = report_search_text(admin_id, request.GET.get('value')).values('id', 'guest_id', 'name', 'phone',
                                                                                 'time', 'remark')
            content = {'status': True, 'message': admin_id, 'content': list(data)}
        except Exception as e:
            content = {'status': False, 'message': str(e), 'content': {}}
...
def delete(request):
    ...
    if admin_id and id:
        try:
            data = report_delete(admin_id=admin_id, id=id)
...
def insert(request):
    ...
    if admin_id:
        ...
        status = 2
        try:
            guest_id = guest_insert(admin_id=admin_id, time=time, name=name, phone=phone, status=status,
            ...
            report = report_insert(guest_id=guest_id, admin_id=admin_id, time=time, name=name, phone=phone,
...
def update(request):
    ...
    if admin_id and report_id:
        try:
            ...
            report = report_update(id=report_id,
                                   admin_id=admin_id,
                                   name=request.GET.get('name'),
                                   time=request.GET.get('time'),
                                   phone=request.GET.get('phone'),
...
def detail(request):
    ...
    if admin_id and report_id:
        try:
            data = report_detail(id=report_id, admin_id=admin_id)
